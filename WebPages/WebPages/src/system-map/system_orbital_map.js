var SystemOrbitalMap = Class.create({
  ...
  buildGUI: function(){
    ...
    var triggerRender = function() {
        if (this.lastFormattedData) {
            this.buildScene()
            this.buildGeometry(this.lastFormattedData)
            this.positionCamera(this.lastFormattedData)
            this.renderer.render(this.scene, this.camera)
        }
    }.bind(this);

    this.GUIParameters.focusBody = 'current vessel';
    var bodyNames = ['current vessel', 'Kerbin', 'Mun', 'Minmus'];
    
    gui.add(this.GUIParameters, 'focusBody', bodyNames).name('Focus On').onChange(function(val) {
        this.cameraSet = false;
        triggerRender();
    }.bind(this));
    
    var f1 = gui.addFolder('Celestial Bodies');
    this.bodyToggles = {};
    for (var i = 1; i < bodyNames.length; i++) {
        var body = bodyNames[i];
        
        // Filter to Kerbin System
        var bInfo = this.datalink.getOrbitalBodyInfo(body);
        if (body !== "Kerbin" && (!bInfo || bInfo.referenceBodyName !== "Kerbin")) continue;

        this.GUIParameters[body] = true;
        this.bodyToggles[body] = f1.add(this.GUIParameters, body).name(body).onChange(triggerRender);
    }
  ...
  buildScene: function(){
    ...
    // Sun light! We will update position based on Sun geometry position.
    this.sunLight = new THREE.PointLight(0xffffff, 1.5, 0, 0); // Intensity 1.5, 0 distance (infinite), 0 decay
  ...
  buildReferenceBodyGeometry: function(formattedData){
    ...
    for (var i = formattedData.referenceBodies.length - 1; i >= 0; i--) {
      ...
      if(info.type == "currentPosition"){
        var material = new THREE.MeshPhongMaterial( { color: color, wireframe: false, shininess: 10 } )
      } else if(info.type == "targetBodyCurrentPosition"){
        var material = new THREE.MeshPhongMaterial( { color: this.targetColor, wireframe: false, shininess: 10 } )
        radius = radius * 1.2
      } else{
        if(info.name != "Sun"){
      ...
      if(info.name == "Sun"){
        this.setPosition(this.sunLight, info.truePosition)
        material = new THREE.MeshBasicMaterial({ color: color, wireframe: false }) // sun should emit, not shade
  ...
  buildManeuverNodeGeometry: function(formattedData){
    for (var i = formattedData.maneuverNodes.length - 1; i >= 0; i--) {
      ...
      // Add a marker at the node position (the start of the maneuver)
      if (maneuverNode.truePosition) {
          var markerGeometry = new THREE.SphereGeometry(this.vehicleLength * 0.4, 16, 16);
          var markerMaterial = new THREE.MeshBasicMaterial({ color: '#ffcc00' }); // Gold/Yellow for nodes
          var marker = new THREE.Mesh(markerGeometry, markerMaterial);
          this.setPosition(marker, maneuverNode.truePosition);
          this.group.add(marker);
          
          // Add a simple label-like glow? No, let's keep it clean first.
      }
  ...
  buildReferenceBodyOrbitPaths: function(formattedData){
    for (var i = formattedData.referenceBodyPaths.length - 1; i >= 0; i--) {
      var pathInfo = formattedData.referenceBodyPaths[i];
      if (this.GUIParameters[pathInfo.referenceBodyName] === false) continue;

      var points = pathInfo.truePositions.map(function(x){ return this.buildVector(x) }.bind(this))
      if (points.length < 2) continue; // skip if not enough points

      var colorToUse = this.orbitPathColors[i % this.orbitPathColors.length];
      var material = new THREE.LineBasicMaterial( { color : colorToUse, linewidth: 2, transparent: true, opacity: 0.6 } );
  ...
  positionCamera: function(formattedData){
    ...
    var vector = this.currentVesselGeometry ? this.currentVesselGeometry.position.clone() : new THREE.Vector3(0,0,0);
    var targetRadius = this.vehicleLength * 2;
    if (this.GUIParameters.focusBody !== 'current vessel') {
        if (this.bodyMeshes && this.bodyMeshes[this.GUIParameters.focusBody]) {
            vector = this.bodyMeshes[this.GUIParameters.focusBody].position.clone();
            if (formattedData && formattedData.referenceBodies) {
                for (var i = 0; i < formattedData.referenceBodies.length; i++) {
                    if (formattedData.referenceBodies[i].name === this.GUIParameters.focusBody) {
                        targetRadius = formattedData.referenceBodies[i].radius * 5 || 250000;
                        break;
                    }
                }
            }
    ...
    if (this.currentVesselGeometry && this.GUIParameters.focusBody === 'current vessel') {
    ...
    if(!this.cameraSet){
      var offset = targetRadius * scaleFactor;
      this.camera.position.set(vector.x + Math.max(offset, this.vehicleLength * scaleFactor * 5), vector.y + Math.max(offset * 1.5, this.vehicleLength * scaleFactor * 10), vector.z + Math.max(offset * 3, this.vehicleLength * scaleFactor * 30))
      ...
      var offset = targetRadius * scaleFactor;
      this.controls.position0 = new THREE.Vector3(vector.x + Math.max(offset, this.vehicleLength * scaleFactor * 5), vector.y + Math.max(offset * 1.5, this.vehicleLength * scaleFactor * 10), vector.z + Math.max(offset * 3, this.vehicleLength * scaleFactor * 30))
    }
  ...
  buildCurveGeometryFromPoints: function(points){
    ...
    geometry.vertices = curve.getPoints( 720 ); // Increased for extra beauty
  ...
  render: function (formattedData) {
    ...
    requestAnimationFrame( function(){
      ...
      this.positionCamera(formattedData)
      this.renderer.render(this.scene, this.camera)
